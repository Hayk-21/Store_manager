...
async def stock(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """What is on the shelf right now, for the store this worker is standing in.

    Reads the same endpoint the item picker uses, so it can only ever show the
    store of the open shift.
    """
    try:
    ...
    if empty:
        ...
        lines.extend(f"• {format.esc(entry['name'])}" for entry in empty)

    # Telegram refuses a message over 4096 characters, and a big catalogue gets
    # there; send it in pieces rather than losing the tail.
