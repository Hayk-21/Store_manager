...
async def test_a_real_evening_end_to_end(client):
    ...
    # dram it had to the wage; there is nothing left to leave behind, and the answer to
    # that is to ask another shop or the owner for money, not to write a bigger number.
...
async def test_the_owners_share_follows_a_sale_added_after_the_count(client):
    ...
    # owner's word, not the worker's: a cashier cannot claim to be leaving more than
    # the books say is in there, and this reading is exactly such a claim — which is
    # the case the owner's box is deliberately left uncapped for.
